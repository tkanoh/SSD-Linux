/*	$ssdlinux: ifconfig.c,v 1.2 2002/12/11 16:44:24 kanoh Exp $	*/
/*
 * ifconfig   This file contains an implementation of the command
 *              that either displays or sets the characteristics of
 *              one or more of the system's networking interfaces.
 *
 * Version:     $Id: ifconfig.c,v 1.2 2002/12/11 16:44:24 kanoh Exp $
 *
 ...
 *	    20011009 - Tamotsu Kanoh, added -l option
 */
...
#ifdef _USAGI_IFCONFIG
    fprintf(stderr, _("  [netmask <address>]  [dstaddr <address>]\n"));
    fprintf(stderr, _("  [[tunnel <address> [src <address>] [proto [any|inet|inet6]]] | -tunnel]\n"));
#else
...
int main(int argc, char **argv)
{
	...
	if (!strcmp(*spp, "tunnel")
		...
		safe_strncpy(host, *spp, (sizeof host));
		if (inet_aftype.input(0, host, (struct sockaddr *) &sin) >= 0) {
		    type = 1;	/* ipv{4,6} over ipv4 */
		} else if (inet6_aftype.input(1, host, (struct sockaddr *) &sa6) >= 0) {
		    type = 2;	/* ipv{4,6} over ipv6 */
		    if (IN6_IS_ADDR_V4MAPPED(&sa6.sin6_addr)) {
			goterr = 1;
		    } else if (IN6_IS_ADDR_V4COMPAT(&sa6.sin6_addr)) {
			fprintf(stderr,
				_("IPv4 address of ipv{4,6} over ipv4 tunnel should be IPv4 address.\n"));
 			type = 1;
			sin.sin_addr.s_addr = sa6.sin6_addr.s6_addr32[3];
		    }
		} else {
		    fprintf(stderr, _("Bad address.\n"));
		    goterr = 1;
		...
		switch(type) {
		case 1:
		    p.iph.daddr = sin.sin_addr.s_addr;
		    break;
		case 2:
		    memcpy(&p6.ip6t_ip6h.ip6_dst, &sa6.sin6_addr, sizeof(struct in6_addr));
		    break;
		}
		spp++;
	    } else if (cmd > 0) {
		...
		case 1:
		    if (inet_aftype.input(0, host, (struct sockaddr *) &sin) < 0) {
			inet_aftype.herror(host);
			goterr = 1;
		    } else {
			p.iph.saddr = sin.sin_addr.s_addr;
		    }
		    break;
		case 2:
			...
			memcpy(&p6.ip6t_ip6h.ip6_src, &sa6.sin6_addr, sizeof(struct in6_addr));
		    }
		    break;
		}
		spp++;
	    ...
	    /* parse protocol */
	    if (*spp && !strcmp(*spp, "proto")) {
		if (!*++spp)
		    usage();
		if (!strcmp(*spp, "inet6")) {
		    proto = IPPROTO_IPV6;
		} else if (!strcmp(*spp, "inet")) {
		    proto = IPPROTO_IPIP;
		} else if (!strcmp(*spp, "any")) {
		    proto = 0;
		} else {
		    fprintf(stderr, _("unsupported protocol : %s\n"), *spp);
		    goterr = 1;
		}
		spp++;
	    }
	    switch(type) {
	    case 1:	/* tunnel over ipv4 mode */
		p.iph.version = 4;
		p.iph.ihl = 5;
		p.iph.protocol = proto;
		p.iph.ttl = ttl;
		ifr.ifr_data = (void *)&p;
		err = ioctl(skfd, cmd > 0 ? SIOCADDTUNNEL : SIOCDELTUNNEL, &ifr);
		break;
	    case 2:	/* tunnel over ipv6 mode */
		p6.ip6t_ip6h.ip6_vfc = 0x60;
		p6.ip6t_ip6h.ip6_plen = 0;
		p6.ip6t_ip6h.ip6_nxt = proto;
		p6.ip6t_ip6h.ip6_hlim = ttl;
		ifr.ifr_data = (void *)&p6;
		err = ioctl(skfd, cmd > 0 ? SIOCADDTUNNEL6 : SIOCDELTUNNEL6, &ifr);
		break;
	    default:
		ifr.ifr_data = NULL;
		err = ioctl(skfd, SIOCDELTUNNEL, &ifr);
		if (err != ENODEV)
		    err = ioctl(skfd, SIOCDELTUNNEL6, &ifr);	/*XXX*/
		break;
	    }
	    if (err < 0) {
