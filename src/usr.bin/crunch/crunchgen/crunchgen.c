/*	$ssdlinux: crunchgen.c,v 1.1.1.1 2002/05/02 13:37:10 kanoh Exp $	*/
...
void gen_outputs(void)
{
    ...
    status("");
    fprintf(stderr, 
	    "Run \"%s -f %s objs exe\" to build crunched binary.\n",
	    makebin,outmkname);
}
...
void prog_makefile_rules(FILE *outmk, prog_t *p)
{
	...
	fprintf(outmk, "\t| %s DBG=\"${DBG}\" -f- depend ${%s_OBJS}\n\n",
	    makebin,p->ident);
    }
    else
    ...
    fprintf(outmk, "\t${LD} -dc -r -o %s.cro %s_stub.o $(%s_OBJPATHS)\n", 
	    p->name, p->name, p->ident);
#ifdef NEW_TOOLCHAIN
    fprintf(outmk, "\t${OBJCOPY} -S -K crunched_%s_stub %s.cro\n",
  	    p->ident, p->name);
#else
    fprintf(outmk, "\t${CRUNCHIDE} -k crunched_%s_stub %s.cro\n", 
	    p->ident, p->name);
#endif
}
