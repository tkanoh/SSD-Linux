/*	$ssdlinux: insmod.c,v 1.7 2003/11/04 08:18:21 yamagata Exp $	*/
/* Insert a module into a running kernel.
  ...
  */
  ...
  /*
     ...
     Default to no exports for architectures with function descriptors.
       Keith Owens <kaos@ocs.com.au> March 2003.
   */
...
/* 
 * This function checks for the case where a module is compiled with gcc 2.x.y and the kernel
 * is compiled with gcc 3.z. This is a known problematic case due to all kinds of ABI dependencies
 * in the kernel on the gcc version.
 * Returns 0 in the ok case, negative in the problem case
 */
static int check_gcc_mismatch(struct obj_file *f, char *filename)
{
	int module_gcc2_compiled = 0;
	int kernel_gcc3_compiled = 0;
	int i;
	char kernel_line[256];
	FILE *file;
	
	for (i = 0; i < HASH_BUCKETS; ++i) {
		struct obj_symbol *sym = f->symtab[i];
		for (sym = f->symtab[i]; sym ; sym = sym->next) {
		    if (strstr(sym->name, "gcc2_compiled."))
			module_gcc2_compiled = 1;
		}
    	}

	/* If the module is all gcc 3 then no need to continue checking */
	if (!module_gcc2_compiled)
		return 0; 
	memset(kernel_line,0,256);	
	file = fopen("/proc/version","r");
	if (!file)
		return 0;
	if (fread(kernel_line,1,255,file)<=0) 
		return 0;
	fclose(file);
	
	if (strstr(kernel_line,"gcc version 3"))
		kernel_gcc3_compiled = 1;
...
int INSMOD_MAIN(int argc, char **argv)
{
	...
	if (check_gcc_mismatch(f, filename))
		goto out;
