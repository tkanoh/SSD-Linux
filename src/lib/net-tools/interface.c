/*	$ssdlinux: interface.c,v 1.1.1.1 2002/05/02 13:37:07 kanoh Exp $	*/
/* Code to manipulate interface information, shared between ifconfig and
   ...
   $Id: interface.c,v 1.1.1.1 2002/05/02 13:37:07 kanoh Exp $
 */
...
int if_fetch(struct interface *ife)
{
	...
	strcpy(ifr.ifr_name, ifname);
	ifr.ifr_data = (void *)&ife->iptunnel;
	if (ife->type == ARPHRD_TUNNEL || 
	    ife->type == ARPHRD_SIT ||
	    ife->type == ARPHRD_IPGRE) {
	    if (ioctl(fd, SIOCGETTUNNEL, &ifr) < 0) {
		memset(&ife->iptunnel, 0, sizeof(ife->iptunnel));
	    }
	} else {
	    memset(&ife->iptunnel, 0, sizeof(ife->iptunnel));
	}
...
#ifdef _USAGI_IFCONFIG
#if HAVE_AFINET6
    if (ife->type == ARPHRD_TUNNEL6) {
	/* IPIP6 tunnel */
	memset(&ifr, 0, sizeof(ifr));
	strcpy(ifr.ifr_name, ifname);
	ifr.ifr_data = (void *)&ife->ip6tunnel;
	if (ioctl(fd, SIOCGETTUNNEL6, &ifr) < 0) {
	    memset(&ife->ip6tunnel, 0, sizeof(ife->ip6tunnel));
	}
    } else {
	memset(&ife->ip6tunnel, 0, sizeof(ife->ip6tunnel));
    }
...
void ife_print_line(struct interface *ptr)
{
	printf("%s\n", ptr->name);
}
...
void ife_print_long(struct interface *ptr)
{
...
#if HAVE_AFINET
    static struct aftype *iptype = NULL;
#endif
#if HAVE_AFIPX
...
#if HAVE_AFINET
    if (ptr->iptunnel.iph.saddr || ptr->iptunnel.iph.daddr) {
	struct sockaddr_in sin;
	const char *proto = "any";
	char protobuf[32];
	if (iptype == NULL)
	    iptype = get_afntype(AF_INET);
	if (iptype) {
	    printf("[");
	    if (ptr->iptunnel.iph.saddr) {
		sin.sin_family = AF_INET;
		sin.sin_addr.s_addr = ptr->iptunnel.iph.saddr;
		printf("%s", iptype->sprint(((struct sockaddr *)&sin), 1));
	    }
	    switch(ptr->iptunnel.iph.protocol) {
	    case 0:
		break;
	    case IPPROTO_IPIP:
		proto = "inet";
		...
		break;
	    default:
		sprintf(protobuf, "unknown(%d)", ptr->iptunnel.iph.protocol);
		proto = protobuf;
	    }
	    printf("-%s->", proto);
	    if (ptr->iptunnel.iph.daddr) {
		sin.sin_family = AF_INET;
		sin.sin_addr.s_addr = ptr->iptunnel.iph.daddr;
		printf("%s", iptype->sprint(((struct sockaddr *)&sin), 1));
	    }
	    printf("]");
	}
...
#if _USAGI_IFCONFIG
#if HAVE_AFINET6
    if (!IN6_IS_ADDR_UNSPECIFIED(&ptr->ip6tunnel.git_ip6h.ip6_src) ||
	!IN6_IS_ADDR_UNSPECIFIED(&ptr->ip6tunnel.git_ip6h.ip6_dst)) {
	...
	if (!IN6_IS_ADDR_UNSPECIFIED(&ptr->ip6tunnel.git_ip6h.ip6_src)) {
	    ...
	    memcpy(&sin6.sin6_addr.s6_addr32, &ptr->ip6tunnel.git_ip6h.ip6_src, 
		   sizeof(struct in6_addr));
	    printf("%s", inet6_aftype.sprint(((struct sockaddr *)&sin6), 1));
	}
	switch(ptr->ip6tunnel.git_ip6h.ip6_nxt) {
	case 0:
	    break;
	case IPPROTO_IPIP:
	    proto = "inet";
	...
	case IPPROTO_IPV6:
	    ...
	    break;
	default:
	    sprintf(protobuf, "unknown(%d)", ptr->ip6tunnel.git_ip6h.ip6_nxt);
	    proto = protobuf;
	}
	printf("-%s->", proto);
	if (!IN6_IS_ADDR_UNSPECIFIED(&ptr->ip6tunnel.git_ip6h.ip6_dst)) {
	    sin6.sin6_family = AF_INET6;
	    memcpy(&sin6.sin6_addr.s6_addr32, &ptr->ip6tunnel.git_ip6h.ip6_dst,
		   sizeof(struct in6_addr));
	    printf("%s", inet6_aftype.sprint(((struct sockaddr *)&sin6), 1));
	}
	printf("]");
